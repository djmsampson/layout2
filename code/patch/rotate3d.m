...
% Make invisible axes with rotate line (which is invisible by default)
localCreateRotateAxesAndLineObj(rdata);
...
lineProps.Parent = rdata.ModeStateData.rotateAxes;

if isHG2   
    rdata.ModeStateData.outlineObj = createLineHGUsingMATLABClasses(rdata.ModeStateData.outlineData(1,:),...
...
function localCreateTextBox(rdata)
...
% Remove rotation axes so they do not get serialized
...
% if ax is empty and we're in HG2, we don't need to check the bounds,
% but in HG1 we do
if ~isempty(ax) && ...
  (~matlab.graphics.internal.isGraphicsVersion1 || ...
   (matlab.graphics.internal.isGraphicsVersion1 && matlab.graphics.interaction.inBounds(ax(1))))
    axes_found = 1;
...
resetplotview(matlab.graphics.interaction.vectorizePlotyyAxes(ax),'InitializeCurrentView');
...
if strcmpi(sel_type,'open')
    ...
    resetplotview(matlab.graphics.interaction.vectorizePlotyyAxes(ax),'ApplyStoredView');
...
% present
enableAxesListeners(rotaObj.ModeStateData.targetAxis, false)
...
%--------------------------------------------------------------------%
% Turn on axes dirty listeners
function enableAxesListeners(hAxes, enable)

% Turn back on axes dirty listeners to dirty the legend when an axes
% property changes
if ~graphicsversion(hAxes,'handlegraphics')
    if isprop(hAxes,'LayoutManager') && isvalid(hAxes.LayoutManager);
        lm = hAxes.LayoutManager;
        lm.enableAxesDirtyListeners(enable); 
    end
end
...
enableAxesListeners(hAxes, true);
...
if ishghandle(ax)
    ...
    if  ((graphicsversion( fig, 'handlegraphics') && any(ishandle(b))) || (~graphicsversion( fig, 'handlegraphics') && isobject(b))) && ~get(b,'Enable')
        % ignore this axes
        ax = [];
...
if ~graphicsversion(hFigure,'handlegraphics')
    propName = 'Point';
    curr_units =  evd.(propName);
else
    propName = 'CurrentPoint';
    curr_units = hgconvertunits(hFigure,[0 0 evd.(propName)],...
...
% if hAx is empty and we're in HG2, we don't need to check the bounds,
% but in HG1 we do
if ~isempty(hAx) && ...
  (~matlab.graphics.internal.isGraphicsVersion1 || ...
   (matlab.graphics.internal.isGraphicsVersion1 && ~matlab.graphics.interaction.inBounds(hAx(1))))
...
if ~graphicsversion(rotaObj.FigureHandle,'handlegraphics')
    propName = 'Point';
    new_pt = hgconvertunits(rotaObj.FigureHandle,[0 0 evd.(propName)],...
...
switch get(obj,'tag')
    case 'Reset';
        ...
        rotaObj.fireActionPreCallback(localConstructEvd(rotaObj.ModeStateData.targetAxis));
        resetplotview(matlab.graphics.interaction.vectorizePlotyyAxes(hAxes),'ApplyStoredView');
        rotaObj.fireActionPostCallback(localConstructEvd(rotaObj.ModeStateData.targetAxis));
    case 'SnapToXY';
        view(hAxes,0,90);
    case 'SnapToXZ';
...
if graphicsversion(fig,'handlegraphics') 
    ...
    allAxes = matlab.graphics.interaction.hitAxes(fig, evd);
end
ax = [];
